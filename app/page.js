          ...
          <div className="bg-white p-10 rounded-[3.5rem] shadow-xl border border-[#B11E48]/5 mt-12">
            ...
            <div className="space-y-4">
              ...
              <button onClick={async () => {
                 ...
                 window.location.href = `https://wa.me/917024185979?text=Gully Verdict! From: ${user.displayName}. Feedback: ${txt}`;
              }} className="w-full bg-[#B11E48] text-white py-6 rounded-[2.5rem] font-black shadow-xl active:scale-95 transition-all">SUBMIT VERDICT</button>
            </div>
          </div>
        </main>
      ) : (
        <main className="max-w-7xl mx-auto pt-16 px-8 grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 space-y-12">
            <div className="flex justify-between items-center mb-8">
                ...
                <div className="flex gap-3">
                    <button onClick={exportToCSV} className="bg-white border-[#B11E48]/20 border px-6 py-3 rounded-2xl text-[10px] font-black uppercase text-[#B11E48] flex items-center gap-2 shadow-sm transition-all active:scale-95"><Download size={14}/> CSV Report</button>
                    <button onClick={addAdmin} className="bg-white border-[#B11E48]/20 border px-6 py-3 rounded-2xl text-[10px] font-black uppercase text-[#B11E48] flex items-center gap-2 shadow-sm transition-all active:scale-95"><Users size={14}/> Partner Access</button>
                </div>
            </div>
              ...
              return (
                <div key={type} className="bg-white p-10 rounded-[4rem] shadow-sm border border-[#B11E48]/5">
                  ...
                  <div className="flex gap-10">
                    <div className="w-1/3 aspect-square bg-[#FFFBEB] rounded-[3.5rem] overflow-hidden border border-[#B11E48]/10 shadow-inner relative group">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><Camera size={32} className="text-[#B11E48]" /></div>
                    </div>
                    <div className="flex-1 space-y-4">
                      <div className="relative group">
                        <LinkIcon className="absolute left-4 top-4 text-stone-300 group-focus-within:text-[#B11E48]" size={16} />
                        <input value={bData.img} onChange={e => setBData({...bData, img: e.target.value})} className="w-full pl-12 p-4 bg-[#FFFBEB]/50 rounded-2xl text-xs font-mono border border-transparent focus:border-[#B11E48]/20 outline-none" placeholder="Paste Photo Direct Link" />
                      </div>
                      ...
                      <input value={bData.tagline} onChange={e => setBData({...bData, tagline: e.target.value})} className="w-full p-4 bg-[#FFFBEB]/50 rounded-2xl text-sm italic border border-transparent focus:border-[#B11E48]/20 outline-none" placeholder="Tagline..." />
          ...
          <div className="bg-white p-10 rounded-[4rem] shadow-sm border border-[#B11E48]/5 flex flex-col h-[850px]">
            ...
            <div className="flex-1 overflow-y-auto space-y-6 pr-2">
              {reviews.length === 0 ? <p className="text-xs text-stone-300 italic text-center">Waiting for the gully to speak...</p> : reviews.map((r, i) => (
                <div key={i} className="p-6 bg-[#FFFBEB]/30 rounded-[2.5rem] border border-[#B11E48]/10 hover:bg-white transition-all">
                  ...
                  <p className="text-[8px] text-stone-300 font-bold text-right uppercase mt-4">{r.date}</p>
                </div>
              ))}
            </div>
          </div>
