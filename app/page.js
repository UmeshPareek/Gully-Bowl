...
import { Download, Users, LayoutDashboard, Utensils, Send, CheckCircle, Link as LinkIcon, Phone, User as UserIcon, MapPin, Briefcase, Camera, LogOut, Calendar, TrendingUp, ShieldCheck, History, ArrowUpDown, Trash2, ShoppingBag, Clock, Plus, Minus, ClipboardList, MessageSquare, Tag, Flame, Zap, Wheat, Navigation, Map, CreditCard, IndianRupee, QrCode } from 'lucide-react';
...
export default function GullyBowlApp() {
  ...
  const exportData = (type) => {
    ...
    const headers = type === 'orders' ? ["Date", "Name", "Mobile", "Address", "Maps Link", "Veg", "NV", "Fee Paid"] : ["Date", "Name", "Mobile", "Veg Review", "Meat Review"];
    const rows = data.map(d => type === 'orders' ? [d.date, d.name, `'${d.phone}`, `"${d.fullAddress}"`, d.mapsLink, d.vegQty, d.nvQty, d.fee] : [d.date, d.name, `'${d.phone}`, `"${d.vegText || d.text}"`, `"${d.nvText}"`]);
  ...
  const handlePlaceOrder = async () => {
    ...
    // Construct UPI Link
    const upiLink = `upi://pay?pa=${paySettings.upi}&pn=GullyBowl&am=${paySettings.fee}&cu=INR`;
    
    await addDoc(collection(db, "orders"), orderData);
    alert(`Order data saved. Redirecting to UPI to pay Delivery Fee: ₹${paySettings.fee}`);
      ...
      {view === 'user' ? (
        <main className="max-w-md mx-auto pt-16 px-6">
          ...
          {!userMode ? (
            <div className="bg-white p-10 rounded-[3.5rem] shadow-2xl border border-[#B11E48]/5 text-center space-y-6">
              <h3 className="text-3xl font-serif font-black italic text-[#B11E48]">The Selection</h3>
              <button onClick={() => setUserMode('order')} className="w-full py-6 bg-[#B11E48] text-white rounded-[2.5rem] font-black shadow-xl flex items-center justify-center gap-3 uppercase tracking-tighter">Order Trial Bowl <ShoppingBag/></button>
              <button onClick={() => setUserMode('review')} className="w-full py-6 bg-white border-2 border-[#B11E48] text-[#B11E48] rounded-[2.5rem] font-black flex items-center justify-center gap-3 uppercase tracking-tighter">Give Verdict <MessageSquare/></button>
            </div>
          ) : (
            <div className="space-y-8 animate-in slide-in-from-bottom-4">
                 ...
                 <div key={item.id} className="bg-white rounded-[3.5rem] shadow-xl border border-[#B11E48]/5 overflow-hidden">
                    <div className="relative aspect-square bg-stone-100">
                        ...
                        <div className="absolute top-6 left-6 px-4 py-2 rounded-full bg-white/90 backdrop-blur shadow-md flex items-center gap-2">
                            ...
                            <span className={`text-[8px] font-black tracking-[0.2em] ${item.txt}`}>{item.tag}</span>
                        </div>
                    </div>
                    <div className="p-8">
               ...
               <div className="bg-white p-10 rounded-[4rem] shadow-2xl border border-[#B11E48]/5 space-y-4 text-center">
                  <h3 className="text-3xl font-serif font-black italic text-[#B11E48]">{userMode === 'order' ? 'Checkout' : 'Verdict'}</h3>
                  {userMode === 'order' ? (
                    <>
                        <div className="bg-green-50 p-4 rounded-3xl mb-4 border border-green-100">
                            <p className="text-[10px] font-black text-green-700 uppercase tracking-tighter">BOWL IS ON US • DELIVERY FEE ₹{paySettings.fee}</p>
                        </div>
                        <div className="space-y-3 text-left">
                            ...
                            <textarea id="o-addr" placeholder="Full Address (Flat No, Apartment) *" className="w-full p-5 bg-[#FFFBEB]/40 rounded-2xl outline-none border border-[#B11E48]/10 h-24"></textarea>
                            <input id="o-maps" placeholder="Google Maps Link" className="w-full p-5 bg-[#FFFBEB]/40 rounded-2xl outline-none border border-[#B11E48]/10" />
                        </div>
                        <button onClick={handlePlaceOrder} className="w-full bg-[#B11E48] text-white py-6 rounded-[2.5rem] font-black shadow-xl uppercase mt-4 flex items-center justify-center gap-3">Pay & Confirm <CreditCard/></button>
                    </>
                  ) : (
                    <>
                        <div className="space-y-3 text-left">
                            ...
                            <textarea id="u-nv-text" placeholder="Meat Verdict..." className="w-full p-5 bg-[#FFFBEB]/40 rounded-3xl h-24 outline-none border border-[#B11E48]/10"></textarea>
                        </div>
                        <button onClick={async () => {
                            ...
                            alert("Verdict Received! 🥣"); setUserMode(null);
        ...
        <main className="max-w-screen-2xl mx-auto pt-16 px-8 flex flex-col">
            <div className="flex justify-between items-start mb-12">
                <div><h2 className="text-5xl font-serif font-black italic text-[#B11E48]">Command Desk</h2>
                ...
                <div className="xl:col-span-1 space-y-8">
                    <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-[#B11E48]/5">
                        ...
                        <div className="space-y-4">
                            <div><p className="text-[8px] font-black uppercase text-stone-300 mb-1">UPI ID</p><input value={paySettings.upi} onChange={e => setPaySettings({...paySettings, upi: e.target.value})} className="w-full p-3 bg-[#FFFBEB] rounded-xl text-[10px] font-mono outline-none border border-transparent focus:border-[#B11E48]/20" /></div>
                            <div><p className="text-[8px] font-black uppercase text-stone-300 mb-1">Fee (₹)</p><input value={paySettings.fee} onChange={e => setPaySettings({...paySettings, fee: e.target.value})} className="w-full p-3 bg-[#FFFBEB] rounded-xl text-lg font-black outline-none" /></div>
                            <button onClick={async () => { await setDoc(doc(db, "settings", "payment"), paySettings); alert("Payment settings live!"); }} className="w-full bg-[#B11E48] text-white py-3 rounded-xl text-[10px] font-black uppercase">Update Payments</button>
                        </div>
                    </div>
                    {['veg', 'nonveg'].map(type => {
                            ...
                            {adminTab === 'orders' ? (
                                allOrders.length === 0 ? <p className="text-center py-20 italic opacity-20 text-stone-400">No orders logged.</p> : allOrders.map((o, i) => (
                                    <div key={i} className="p-6 bg-[#FFFBEB]/50 rounded-[2.5rem] border border-[#B11E48]/5 flex flex-col gap-4">
                                        <div className="flex justify-between items-center">
                                            ...
                                            <div className="flex gap-4">{o.vegQty > 0 && <span className="bg-green-50 text-green-700 px-6 py-2 rounded-full text-xs font-black shadow-sm">VEG: {o.vegQty}</span>}{o.nvQty > 0 && <span className="bg-red-50 text-red-700 px-6 py-2 rounded-full text-xs font-black shadow-sm">MEAT: {o.nvQty}</span>}</div>
                                        </div>
                                        <div className="pt-4 border-t border-stone-100 flex justify-between items-center">
                                            <div className="flex items-start gap-4 flex-1">
                                                ...
                                                <div>
                                                    ...
                                                    {o.mapsLink && <a href={o.mapsLink} target="_blank" className="inline-flex items-center gap-2 mt-2 text-[10px] font-black text-white bg-[#B11E48] px-4 py-2 rounded-lg"><Map size={12}/> OPEN MAPS</a>}
                                                </div>
                                            </div>
                                            <div className="bg-green-100 px-4 py-2 rounded-2xl border border-green-200 text-center shadow-sm">
                                                <p className="text-[8px] font-black text-green-800 uppercase">Paid Fee</p>
                                ...
                                reviews.length === 0 ? <p className="text-center py-20 italic opacity-20 text-stone-400">No gossip yet.</p> : reviews.map((r, i) => (
                                    <div key={i} className="p-8 bg-[#FFFBEB]/30 rounded-[3.5rem] border border-[#B11E48]/10 hover:bg-white transition-all shadow-sm">
                                        <div className="flex justify-between items-start mb-6">
                                            <div><p className="font-black text-xl text-[#B11E48] uppercase tracking-tighter">{r.name}</p><p className="text-[10px] font-bold text-stone-400">{r.phone} • {r.hustle || "Sampler"}</p></div>
