...
// Standard login method to avoid mobile "initial state" errors
...
export default function GullyBowlApp() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (u) => {
      if (u) {
        setUser(u);
        // Sync Admin list from Firestore
  ...
  const handleLogin = async () => {
    try {
      ...
      console.error(error);
      alert("Mobile Login: If the popup didn't open, please allow popups in your browser settings.");
    }
  };
  ...
  const exportToExcel = () => {
    ...
    const rows = reviews.map(r => [
        r.date, 
        r.name, 
        `'${r.phone || ""}`, // Fixed undefined mobile: captures 'phone' field
        r.hustle || "", 
        r.hood || "", 
        r.tried || "", 
        `"${(r.text || "").replace(/"/g, '""')}"`
  ...
  const addAdmin = async () => {
    const email = prompt("Enter Partner Email for Admin Access:");
  ...
  const submitVerdict = async () => {
    ...
    if (!phone || !hustle || !hood || !text) return alert("Wait! All fields are required for the Gully Verdict. 🥗");

    const reviewData = { 
        name: user.displayName, 
        phone, // Captures 'phone' to prevent undefined in report
        hustle, 
        hood, 
        tried: selectedTab, 
        text, 
  ...
  if (!user) return (
    <div className="min-h-screen bg-[#FFFBEB] flex items-center justify-center p-6">
      <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl text-center max-w-sm w-full border border-[#B11E48]/10">
        ...
        <button onClick={handleLogin} className="w-full py-4 bg-[#B11E48] text-white rounded-2xl font-black shadow-xl hover:scale-105 transition-all">SIGN INTO THE GULLY</button>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#FFFBEB] text-[#1A1A1A] pb-32">
      {isAdmin && (
        <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex gap-4 bg-[#B11E48] text-white p-2 rounded-full shadow-2xl">
          <button onClick={() => setView('user')} className={`px-6 py-3 rounded-full text-xs font-bold ${view === 'user' ? 'bg-white text-[#B11E48]' : ''}`}>Consumer</button>
        ...
        <main className="max-w-md mx-auto pt-16 px-6">
          ...
          <div className="bg-white p-10 rounded-[3.5rem] shadow-xl border border-[#B11E48]/5 mt-12">
            ...
            <div className="space-y-4">
              ...
              <input id="u-phone" type="tel" placeholder="Mobile Number *" className="w-full p-5 bg-[#FFFBEB]/40 rounded-2xl outline-none border border-[#B11E48]/10 focus:ring-1 ring-[#B11E48]" />
              <input id="u-hustle" placeholder="Your Hustle? *" className="w-full p-5 bg-[#FFFBEB]/40 rounded-2xl outline-none border border-[#B11E48]/10 focus:ring-1 ring-[#B11E48]" />
              <input id="u-hood" placeholder="Which Hood? *" className="w-full p-5 bg-[#FFFBEB]/40 rounded-2xl outline-none border border-[#B11E48]/10 focus:ring-1 ring-[#B11E48]" />
              <textarea id="u-text" placeholder="Be raw. Be Gully. *" className="w-full p-5 bg-[#FFFBEB]/40 rounded-3xl h-32 outline-none border border-[#B11E48]/10 focus:ring-1 ring-[#B11E48]"></textarea>
        ...
        <main className="max-w-7xl mx-auto pt-16 px-8 flex flex-col">
            <div className="w-full flex justify-between items-center mb-12">
                ...
                <div className="flex gap-4">
                    <button onClick={addAdmin} className="bg-white border-[#B11E48]/20 border px-6 py-3 rounded-2xl text-[10px] font-black uppercase text-[#B11E48] flex items-center gap-2 shadow-sm transition-all active:scale-95"><Users size={14}/> Add Admin</button>
                    <button onClick={exportToExcel} className="bg-white border-[#B11E48]/20 border px-6 py-3 rounded-2xl text-[10px] font-black uppercase text-[#B11E48] flex items-center gap-2 shadow-sm transition-all active:scale-95"><Download size={14}/> Report</button>
                    <button onClick={() => signOut(auth)} className="bg-[#B11E48] text-white p-3 rounded-2xl shadow-md"><LogOut size={18}/></button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                <div className="lg:col-span-2 space-y-10">
                    {['veg', 'nonveg'].map(type => {
                      ...
                      return (
                        <div key={type} className="bg-white p-10 rounded-[4rem] shadow-sm border border-[#B11E48]/5 transition-all">
                            <div className="flex justify-between items-center mb-8">
                                <p className={`text-xs font-black uppercase tracking-widest ${type === 'veg' ? 'text-green-600' : 'text-[#B11E48]'}`}>{type} Editor</p>
                                <button onClick={() => setDoc(doc(db, "menu", type), bData)} className="bg-[#B11E48] text-white px-8 py-3 rounded-2xl text-[10px] font-black shadow-lg hover:scale-105 transition-all">PUBLISH {type.toUpperCase()} LIVE</button>
                            </div>
                            <div className="flex gap-10">
                                <div className="w-1/3 aspect-square bg-[#FFFBEB] rounded-[3.5rem] overflow-hidden border border-[#B11E48]/10 shadow-inner group relative">
                                    <img src={bData.img || "https://images.unsplash.com/photo-1546069901-ba9599a7e63c"} className="w-full h-full object-cover transition-opacity group-hover:opacity-40" />
                                    <Camera size={24} className="absolute inset-0 m-auto text-[#B11E48] opacity-0 group-hover:opacity-100 transition-all" />
                                </div>
                                <div className="flex-1 space-y-4">
                                    <input value={bData.img} onChange={e => setBData({...bData, img: e.target.value})} className="w-full p-4 bg-[#FFFBEB]/50 rounded-2xl text-xs font-mono outline-none border border-transparent focus:border-[#B11E48]/20" placeholder="PASTE PHOTO URL (.JPG)" />
                                    <input value={bData.name} onChange={e => setBData({...bData, name: e.target.value})} className="w-full p-4 bg-[#FFFBEB]/50 rounded-2xl font-bold outline-none border border-transparent focus:border-[#B11E48]/20" placeholder="Bowl Name" />
                                    <input value={bData.tagline} onChange={e => setBData({...bData, tagline: e.target.value})} className="w-full p-4 bg-[#FFFBEB]/50 rounded-2xl text-sm italic outline-none border border-transparent focus:border-[#B11E48]/20" placeholder="Tagline..." />
                ...
                <div className="bg-white p-10 rounded-[4rem] shadow-sm border border-[#B11E48]/5 h-[850px] flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
                        {reviews.length === 0 ? <p className="text-xs text-stone-300 italic text-center">Gully silence...</p> : reviews.map((r, i) => (
