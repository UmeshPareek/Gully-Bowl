"use client"; import React, { useState, useEffect } from 'react'; import { initializeApp } from "firebase/app"; import { getAuth, signInWithPopup, GoogleAuthProvider, onAuthStateChanged, signOut } from "firebase/auth"; import { getFirestore, doc, setDoc, collection, addDoc, onSnapshot, query, orderBy, limit, where } from "firebase/firestore"; import { Download, Users, LayoutDashboard, Utensils, Send, CheckCircle, Link as LinkIcon, Phone, User as UserIcon, MapPin, Briefcase, Camera, LogOut, Calendar, TrendingUp, ShieldCheck, History, Filter, ArrowUpDown, Trash2 } from 'lucide-react'; 

const firebaseConfig = { apiKey: "AIzaSyC_7TR7XJwZDtOf2NytiJzaKlqnDApZDDY", authDomain: "gully-bowl.firebaseapp.com", projectId: "gully-bowl", storageBucket: "gully-bowl.firebasestorage.app", messagingSenderId: "141914833931", appId: "1:141914833931:web:c8adafe2c157b88160730e" }; 
...
const SUPER_ADMIN = "pareeku01@gmail.com"; const INITIAL_ADMINS = [SUPER_ADMIN]; 

export default function GullyBowlApp() { const [user, setUser] = useState(null); const [isAdmin, setIsAdmin] = useState(false); const [view, setView] = useState('user'); const [selectedTab, setSelectedTab] = useState('Veg'); const [adminList, setAdminList] = useState(INITIAL_ADMINS); 

const [vegData, setVegData] = useState({ name: "Gully Green", tagline: "Gourmet Soul", p: "0", f: "0", c: "0", img: "" }); const [nvData, setNvData] = useState({ name: "Gully Meat", tagline: "Street Flavors", p: "0", f: "0", c: "0", img: "" }); const [reviews, setReviews] = useState([]); const [logs, setLogs] = useState([]); const [activeTrialDate, setActiveTrialDate] = useState(new Date().toLocaleDateString('en-GB')); const [sortOrder, setSortOrder] = useState('desc'); // 'desc' for Newest, 'asc' for Oldest const [adminSearch, setAdminSearch] = useState(""); 

const BRAND_LOGO = "https://raw.githubusercontent.com/UmeshPareek/Gully-Bowl/main/Gully%20Bowl%20Logo%20(2).png"; 

useEffect(() => { const unsubscribe = onAuthStateChanged(auth, (u) => { if (u) { setUser(u); onSnapshot(doc(db, "settings", "admins"), (d) => { let list = d.exists() ? d.data().emails : INITIAL_ADMINS; if (!list.includes(SUPER_ADMIN)) list = [SUPER_ADMIN, ...list]; setAdminList(list); if (list.includes(u.email)) setIsAdmin(true); }); } }); 
...
const logAction = async (action) => { await addDoc(collection(db, "logs"), { admin: user.email, action: action, timestamp: Date.now(), date: new Date().toLocaleString() }); }; 

const handleAdminManagement = async (email, action) => { if (user.email !== SUPER_ADMIN) return alert("Only the Super Admin can manage other admins."); if (email === SUPER_ADMIN) return alert("Super Admin status is permanent."); 
...
const exportToExcel = () => { if (reviews.length === 0) return alert("No verdicts to export!"); const headers = ["Date", "Trial Date", "Customer Name", "Mobile", "Hustle", "Hood", "Tried", "Veg Review", "Non-Veg Review"]; const rows = reviews.map(r => [r.date, r.trialDate || r.date.split(',')[0], r.name, '${r.phone || ""}, r.hustle || "", r.hood || "", r.tried || "", "${(r.vegText || "").replace(/"/g, '""')}", "${(r.nvText || "").replace(/"/g, '""')}"]); const csvContent = "\uFEFF" + [headers, ...rows].map(e => e.join(",")).join("\n"); const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = Gully_Bowl_Master_Report.csv; a.click(); }; 

const submitVerdict = async () => { const phone = document.getElementById('u-phone').value.trim(); const hustle = document.getElementById('u-hustle').value.trim(); const hood = document.getElementById('u-hood').value.trim(); let vegText = document.getElementById('u-veg-text')?.value.trim() || ""; let nvText = document.getElementById('u-nv-text')?.value.trim() || ""; 
...
<button onClick={() => signInWithPopup(auth, provider)} className="w-full py-4 bg-[#B11E48] text-white rounded-2xl font-black shadow-xl">ENTER THE GULLY  

); 

return (  

{isAdmin && ( <button onClick={() => setView('user')} className={px-6 py-3 rounded-full text-xs font-bold transition-all ${view === 'user' ? 'bg-white text-[#B11E48]' : ''}}>Consumer <button onClick={() => setView('admin')} className={px-6 py-3 rounded-full text-xs font-bold transition-all ${view === 'admin' ? 'bg-white text-[#B11E48]' : ''}}>Admin Desk )} 
