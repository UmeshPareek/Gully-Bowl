...
export default function GullyBowlApp() {
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] text-[#1A1A1A] pb-32">
      {isAdmin && (
        <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex gap-4 bg-black/90 text-white p-2 rounded-full shadow-2xl backdrop-blur-md">
          <button onClick={() => setView('user')} className={`flex items-center gap-2 px-6 py-3 rounded-full text-xs font-bold transition-all ${view === 'user' ? 'bg-[#B11E48] text-white' : 'hover:bg-white/10'}`}><Utensils size={14}/> Consumer View</button>
          <button onClick={() => setView('admin')} className={`flex items-center gap-2 px-6 py-3 rounded-full text-xs font-bold transition-all ${view === 'admin' ? 'bg-[#B11E48] text-white' : 'hover:bg-white/10'}`}><LayoutDashboard size={14}/> Admin Desk</button>
        </nav>
      )}
        ...
        <main className="max-w-7xl mx-auto pt-16 px-8 grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 space-y-12">
            ...
            {['veg', 'nonveg'].map(type => {
              ...
              return (
                <div key={type} className="bg-white p-10 rounded-[4rem] shadow-sm border border-stone-100">
                  <div className="flex justify-between items-center mb-8">
                    <p className={`text-xs font-black uppercase tracking-[0.3em] ${type === 'veg' ? 'text-green-600' : 'text-red-600'}`}>{type} Manager</p>
                    <button onClick={() => publishUpdate(type)} className="bg-[#B11E48] text-white px-8 py-3 rounded-2xl text-[10px] font-black flex items-center gap-2">PUBLISH LIVE <CheckCircle size={14}/></button>
                  </div>
                  <div className="flex gap-10">
                    <div className="w-1/3 aspect-square bg-stone-50 rounded-[3.5rem] overflow-hidden border">
